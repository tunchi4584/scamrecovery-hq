-- Ensure admin balance updates are never overridden by case status changes
-- Update the trigger to preserve admin-set recovery amounts
...
BEGIN
    ...
    DO UPDATE SET
        ...
        -- IMPORTANT: Do NOT update amount_recovered here - preserve admin-set values
        total_cases = EXCLUDED.total_cases,
        completed_cases = EXCLUDED.completed_cases,
        pending_cases = EXCLUDED.pending_cases,
        -- IMPORTANT: Do NOT update recovery_notes here - preserve admin-set values
        updated_at = now();
