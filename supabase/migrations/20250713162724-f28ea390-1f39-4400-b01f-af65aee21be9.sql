-- Remove ALL existing triggers to prevent conflicts
...
UPDATE public.cases 
SET case_number = 'CASE-' || TO_CHAR(NOW(), 'YYYY') || '-' || LPAD(EXTRACT(EPOCH FROM NOW())::TEXT, 10, '0') || '-' || SUBSTRING(id::TEXT, 1, 8)
WHERE case_number IS NULL;

-- Make case_number NOT NULL
ALTER TABLE public.cases ALTER COLUMN case_number SET NOT NULL;

-- Fix any NULL values in balances first
UPDATE public.balances 
SET 
    amount_recovered = COALESCE(amount_recovered, 0),
    total_cases = COALESCE(total_cases, 0),
    completed_cases = COALESCE(completed_cases, 0),
    pending_cases = COALESCE(pending_cases, 0)
WHERE amount_recovered IS NULL OR total_cases IS NULL OR completed_cases IS NULL OR pending_cases IS NULL;
...
UPDATE public.balances 
SET 
    amount_lost = case_stats.total_lost,
    total_cases = case_stats.total_cases,
    completed_cases = case_stats.completed_cases,
    pending_cases = case_stats.pending_cases,
    updated_at = NOW()
FROM case_stats 
WHERE balances.user_id = case_stats.user_id;

-- Create Atomic Case Creation Function
